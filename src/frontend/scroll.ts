...
(function () {
  ...
  function syncGutterToTextarea(textarea) {
    ...
    if (inner && textarea) {
      ...
      syncVirtualGutterMetrics(textarea);
    }
    updateGutterCurrentLine();
    scheduleVirtualGutterMetrics(textarea);
  }
  ...
  function ensureCurrentLineBar() {
    let bar = document.getElementById("source-current-line");
    if (!bar) {
      bar = document.createElement("div");
      bar.id = "source-current-line";
      bar.className = "source-current-line";
  ...
  function hideCurrentLineBar() {
    const bar = document.getElementById("source-current-line");
    if (bar) bar.style.display = "none";
  }

  function layoutCurrentLineBar(
    ta: HTMLTextAreaElement,
    inner: Element,
    line: number,
  ) {
    const host = ta.closest(".source-code-editor");
    const bar = ensureCurrentLineBar();
    const lineEl = inner.querySelector(
      '.gutter-line[data-line="' + line + '"]',
    ) as HTMLElement | null;
    if (!host || !lineEl) {
    ...
    const top = Math.max(lineRect.top, hostRect.top);
    const bottom = Math.min(lineRect.bottom, hostRect.bottom);
    if (bottom <= top + 0.5) {
      bar.style.display = "none";
      return;
    }
    ...
    bar.style.left = hostRect.left + "px";
    bar.style.width = hostRect.width + "px";
  ...
  function updateGutterCurrentLine() {
    ...
    const current = inner.querySelector(".gutter-line.is-current");
    if (!current || current.getAttribute("data-line") !== String(line)) {
      if (current) current.classList.remove("is-current");
      const next = inner.querySelector(
        '.gutter-line[data-line="' + line + '"]',
      );
      if (next) next.classList.add("is-current");
      _gutterCurrentLine = line;
    }
    layoutCurrentLineBar(ta, inner, line);
  }

  let _gutterMirror: HTMLTextAreaElement | null = null;

  function getOrCreateGutterMirror(): HTMLTextAreaElement {
  ...
  function copyTextareaTypo(dest, style) {
    ...
    (dest.style as CSSStyleDeclaration & { MozTabSize?: string }).MozTabSize =
      style.tabSize;
  ...
  const PROBE_LINES = 32;
  let _cachedLineHeight = 0;
  let _virtualMetricsRaf = 0;
  ...
  function measureProbeLineHeight(mirror, style) {
    ...
    mirror.value = Array(PROBE_LINES).fill("x").join("\n");
    const probe = mirror.scrollHeight / PROBE_LINES;
  ...
  function textareaUsedLineHeight(textarea, style) {
    if (_cachedLineHeight > 0) return _cachedLineHeight;
    const mirror = getOrCreateGutterMirror();
    prepareGutterMirror(mirror, textarea, style);
    _cachedLineHeight = measureProbeLineHeight(mirror, style);
  ...
  function applyGutterRowHeight(el, wrapCount, style, lh) {
    const rowLh = lh > 0 ? lh : usedLineHeightPx(style);
    const h = Math.max(1, wrapCount) * rowLh;
    el.style.fontSize = style.fontSize;
    el.style.fontFamily = style.fontFamily;
    el.style.height = h + "px";
    el.style.minHeight = h + "px";
    el.style.maxHeight = h + "px";
    el.style.lineHeight = rowLh + "px";
    el.style.flexGrow = "0";
    el.style.flexShrink = "0";
    el.style.flexBasis = h + "px";
  }

  function syncVirtualGutterMetrics(textarea) {
    if (!textarea) return;
    const gutter = document.getElementById("source-line-gutter");
    const inner = document.getElementById("source-line-gutter-inner");
    if (!gutter || !inner) return;
    if (gutter.getAttribute("data-virtual-gutter") !== "1") return;

    const style = window.getComputedStyle(textarea);
    const lh = textareaUsedLineHeight(textarea, style);
    const lineCount = Math.max(1, textarea.value.split("\n").length);
    inner.style.fontSize = style.fontSize;
    inner.style.fontFamily = style.fontFamily;
    inner.style.height = lineCount * lh + "px";
    inner.style.flexGrow = "0";
    inner.style.flexShrink = "0";
    inner.style.boxSizing = "border-box";

    const lineEls = inner.querySelectorAll(".gutter-line");
    if (lineEls.length === 0) return;

    const firstLine = parseInt(lineEls[0].getAttribute("data-line") || "1", 10);
    const lastLine = parseInt(
      lineEls[lineEls.length - 1].getAttribute("data-line") || "1",
      10,
    );
    const kids = inner.children;
    if (kids[0] && !kids[0].classList.contains("gutter-line")) {
      (kids[0] as HTMLElement).style.height = (firstLine - 1) * lh + "px";
    }
    for (let i = 0; i < lineEls.length; i++) {
      applyGutterRowHeight(lineEls[i], 1, style, lh);
    }
    const last = kids[kids.length - 1];
    if (last && !last.classList.contains("gutter-line")) {
      (last as HTMLElement).style.height = (lineCount - lastLine) * lh + "px";
    }
  }

  function scheduleVirtualGutterMetrics(textarea) {
    if (!textarea || _virtualMetricsRaf) return;
    const gutter = document.getElementById("source-line-gutter");
    if (!gutter || gutter.getAttribute("data-virtual-gutter") !== "1") return;
    _virtualMetricsRaf = requestAnimationFrame(function () {
      _virtualMetricsRaf = 0;
      syncVirtualGutterMetrics(textarea);
      updateGutterCurrentLine();
    });
  }
  ...
  function syncGutterLineHeights(textarea) {
    ...
    if (!inner) return;
    const gutter = document.getElementById("source-line-gutter");
    if (gutter && gutter.getAttribute("data-virtual-gutter") === "1") {
      syncVirtualGutterMetrics(textarea);
      return;
    }
    ...
    if (!lineWrap) {
      for (let i = 0; i < count; i++) {
        applyGutterRowHeight(gutterLines[i], 1, style, lh);
      }
      return;
    }
    ...
    const probeLh = textareaUsedLineHeight(textarea, style);
    const lines = textarea.value.split("\n");

    for (let i = 0; i < count; i++) {
      const lineText = i < lines.length ? lines[i] : "";
      const grow = measureWrapCount(mirror, lineText, probeLh);
      applyGutterRowHeight(gutterLines[i], grow, style, probeLh);
    }
  }
  ...
  function installTextareaValueGuard(textarea) {
    ...
    if (!nativeDescriptor || !nativeDescriptor.set || !nativeDescriptor.get)
      return;
...
(function () {
  ...
  let _lastScrollerEl: HTMLElement | null = null;
  interface TocPoint {
    x: number;
    y: number;
    id: string;
  ...
  let _tocPoints: TocPoint[] = [];
  let _tocLengths: number[] = [];
  ...
  function parseTocMeta(wrapper) {
    ...
    const rowHeight = parseFloat(
      wrapper.getAttribute("data-toc-row-height") || "28",
    );
    let levels = [];
    try {
  ...
  function computeTocPointsFromMeta(wrapper) {
    ...
    if (inner) {
      ...
      const sample = document.querySelector(
        "#toc-virtual-list .toc-item[data-toc-index] .toc-node-bullet",
      );
      if (sample) {
        const item = sample.closest(".toc-item");
        const idx = parseInt(
          (item as Element).getAttribute("data-toc-index") || "0",
          10,
        );
  ...
  function refreshTocTreePathImpl() {
    ...
    const trackPath = document.getElementById(
      "toc-track-path",
    ) as SVGPathElement | null;
    const fillPath = document.getElementById(
      "toc-progress-fill-path",
    ) as SVGPathElement | null;
    const headPip = document.getElementById(
      "toc-progress-head",
  ...
  function updateTocFill() {
    ...
    if (k >= 0 && k < _tocLengths.length) {
      ...
      const L_next =
        k + 1 < _tocLengths.length ? _tocLengths[k + 1] : _totalTocLength;
  ...
  function setupTocObservers() {
    ...
    if (!tocWrapper || (tocWrapper as HTMLElement).dataset.tocObserved === "1")
      return;
  ...
  function computeActiveHeadingAndFractionFromHtml(scrollArea) {
    const headings = scrollArea.querySelectorAll(
      ".doc-heading, h1[id], h2[id], h3[id], h4[id], h5[id], h6[id]",
    );
    ...
    if (activeIdx < headings.length - 1) {
      ...
      if (span > 1) {
        fraction = Math.min(
          1.0,
          Math.max(0.0, (offsetThreshold - topCurrent) / span),
        );
      }
    }
  ...
  function computeActiveHeadingAndFractionFromTextarea(textarea) {
    ...
    let headings: { line: number; id: string }[] = [];
  ...
  function applyTocHighlight(info) {
    if (!info || !info.activeId) {
      document
        .querySelectorAll("#toc-virtual-list .toc-item")
        .forEach((item) => {
    ...
    let activeElem: Element | null = null;
    ...
    if (info.activeId !== _lastActiveHeadingId) {
      ...
      if (!window._userIsHoveringToc) {
        ...
        if (tocScroller && tocWrapper && resolvedIdx >= 0) {
          const rowHeight = parseFloat(
            tocWrapper.getAttribute("data-toc-row-height") || "28",
          );
