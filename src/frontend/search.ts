...
window.highlightSearchMatches = function (query) {
  try {
    ...
    const root =
      document.querySelector(".app-main-viewer") ||
    ...
    function walkTextNodes(node, callback) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (
          ["SCRIPT", "STYLE", "BUTTON", "INPUT", "HEADER", "NAV"].includes(
            el.tagName,
          ) ||
          el.classList.contains("app-titlebar") ||
          el.classList.contains("app-toolbar")
        ) {
          return;
        }
      ...
      while (index !== -1) {
        if (index > lastIdx) {
          fragment.appendChild(
            document.createTextNode(text.substring(lastIdx, index)),
          );
        }
...
window.updateSearchCountUI = function (current, total) {
  try {
    ...
    if (el) {
      if (total === 0) {
        el.innerText =
          window._searchState && window._searchState.query ? "0 results" : "";
      } else {
