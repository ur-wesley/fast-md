...
(function () {
  function updateToolbarActiveStates() {
    ...
    if (
      ...
      wysiwygSurface.contains(selection.anchorNode)
    ) {
      ...
      while (node && node !== wysiwygSurface) {
        ...
        if (tag === "ul") {
          if (el.classList && el.classList.contains("task-list")) {
        ...
        if (tag === "table" || tag === "td" || tag === "th" || tag === "tr")
          activeTools.add("table");
        if (tag === "a") activeTools.add("link");
        if (
          tag === "div" &&
          el.classList &&
          el.classList.contains("mdx-callout")
        )
    ...
    if (
      ...
      if (trimmedLine.startsWith("# ") || /^#\s/.test(trimmedLine))
        activeTools.add("h1");
      else if (trimmedLine.startsWith("## ") || /^##\s/.test(trimmedLine))
        activeTools.add("h2");
      else if (trimmedLine.startsWith("### ") || /^###\s/.test(trimmedLine))
        activeTools.add("h3");
      else if (trimmedLine.startsWith("> ") || trimmedLine === ">")
        activeTools.add("quote");
    ...
    document.addEventListener(evtName, (e) => {
      if (
        ...
        ((e.target as HTMLElement).id === "wysiwyg-editor-surface" ||
          (e.target as HTMLElement).id === "source-markdown-textarea" ||
