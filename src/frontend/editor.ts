...
function findWordBoundaries(line, cursorOffset) {
  ...
  if (
    pos > 0 &&
    pos === line.length &&
    /[\w\u00C0-\u024F\-]/.test(line.charAt(pos - 1))
  ) {
    pos--;
  } else if (
    pos < line.length &&
    !/[\w\u00C0-\u024F\-]/.test(line.charAt(pos)) &&
    pos > 0 &&
    /[\w\u00C0-\u024F\-]/.test(line.charAt(pos - 1))
  ) {
    pos--;
...
window.wrapSourceSelection = function (prefix, suffix, defaultText) {
  ...
  if (start < end) {
    ...
    if (isDirectMatch) {
      ...
      if (window.pushEditorHistory)
        window.pushEditorHistory(
          ta.value,
          ta.selectionStart,
          ta.selectionEnd,
          true,
    ...
    if (start >= pLen && end + sLen <= val.length) {
      ...
      if (isOuterMatch) {
        ta.value =
          val.substring(0, start - pLen) + selected + val.substring(end + sLen);
    ...
    if (isItalic) {
      ...
      const regex = new RegExp(
        escPfx + "([^" + escapeRegex(prefix.charAt(0)) + "\n]+?)" + escPfx,
        "g",
      );
  ...
  // ==========================================
  let foundMatch: { start: number; end: number; inner: string } | null = null;
  ...
  if (foundMatch) {
    ...
    ta.value =
      val.substring(0, foundMatch.start) +
      foundMatch.inner +
      val.substring(foundMatch.end);
    const newCursor = Math.max(
      lineStart,
      Math.min(ta.value.length, start - pLen),
    );
  ...
  if (word.text.length > 0) {
    ...
    ta.value =
      val.substring(0, wordAbsStart) + wrappedWord + val.substring(wordAbsEnd);
...
window.insertSourceLinePrefix = function (prefix) {
  ...
  const isListOrQuote =
    prefix.startsWith("-") || prefix.startsWith("1.") || prefix.startsWith(">");

  let oldPrefixMatch: string | null = null;
  let oldPrefixLen = 0;
  ...
  const newCursor = Math.max(
    lineStart,
    Math.min(ta.value.length, start + cursorShift),
  );
  ta.selectionStart = newCursor;
  ta.selectionEnd = newCursor;
  ta.dispatchEvent(new Event("input", { bubbles: true }));
...
window.handleSourceTab = function (e) {
  ...
  if (e.shiftKey) {
    ...
    const newVal =
      ...
      newLine +
      val.substring(lineStart + line.length);
...
function handleListEnter(ta, val, start) {
  ...
  if (contentOnly.trim() === "") {
    const newVal =
      val.substring(0, lineStart) + indent + val.substring(lineEnd);
  ...
  const newCursor =
    lineStart + currentLine.length + 1 + indent.length + nextMarker.length;
...
function handleTableEnter(ta, val, start) {
  ...
  const afterLine =
    lineEnd < val.length && val[lineEnd] === "\n" ? lineEnd + 1 : lineEnd;
  const nextNl = val.indexOf("\n", afterLine);
  const nextLine = val.substring(
    afterLine,
    nextNl === -1 ? val.length : nextNl,
  );
