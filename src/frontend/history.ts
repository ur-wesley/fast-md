...
(function () {
  ...
  window.editorUndo = function () {
    ...
    if (
      wysiwygSurface &&
      (document.activeElement === wysiwygSurface ||
        wysiwygSurface.contains(document.activeElement))
    ) {
    ...
    if (
      historyIndex === history.length - 1 &&
      history[historyIndex] &&
      history[historyIndex].value !== ta.value
    ) {
      initOrPushHistory(ta.value, ta.selectionStart, ta.selectionEnd, true);
    }
  ...
  document.addEventListener("input", (e) => {
    if (
      e.target &&
      (e.target as HTMLTextAreaElement).id === "source-markdown-textarea"
    ) {
  ...
  document.addEventListener("keydown", (e) => {
    if ((e.ctrlKey || e.metaKey) && !e.altKey) {
      ...
      if (
        ta &&
        (document.activeElement === ta || ta.contains(document.activeElement))
      ) {
        if (e.key === "z" || e.key === "Z") {
          e.preventDefault();
          if (e.shiftKey) {
